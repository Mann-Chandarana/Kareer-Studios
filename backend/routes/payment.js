  ...
  try {
    ...
    Instamojo.getPaymentDetails(
      ...
      async (err, response) => {
        ...
        if (
          response.success &&
          response.payment_request.status === "Completed"
        ) {
          try {
            const { buyer_email, buyer_phone, buyer_name, amount, fees } =
              response.payment_request.payment;
            ...
            await receiptHandler.addReceipt(
              id,
              student_id,
              Number(amount),
              buffer
